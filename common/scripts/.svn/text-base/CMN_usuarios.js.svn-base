//TITULOS
var titulos = new Array("Informaci&oacute;n Adicional de Usuarios","Asignaci&oacute;n de perfiles a usuarios");
//DML
var dmlBuscarMaster = "1105";
var dmlnuevoM = "1101";
var dmleditarM = "1102";
...
var dmlNuevoDetail = "1111";
var dmleditarD = "xx";
var dmlbuscarUnicoD = "xx"; 
var dmleliminarD = "1113";
var dmlGrabarDetail = "";
...
var columnasD = new Array("usuario_id2","perfil_id","nombre","descripcion","esquema");
var tiposD = new Array("text","text","text","text","text");
//DESCRIPCIONES
var descripcionesM = new Array("Id Usuario","Clave","Apellido","Nombre");
var descripcionesD = new Array("Id Usuario","Id Perfil","Nombre","Descripci&oacute;n","Pa&iacute;s");
//PRIMARY KEY
...
var dojoTypeFormD = new Array('FilteringSelect','FilteringSelect');
var searchSelectFormD = new Array('usuario_id2','detalle');
var selectFormD = new Array('1118','1116');
...
var layoutDetail = [
	...
	{width:"auto",field:columnasD[4],name:descripcionesD[4],hidden:true}];
//campos de auditoria
...
function NuevoDetail(){
	//Creacion del contenedor
	var divCamposD = new dijit.layout.BorderContainer({style:'width:350px;text-align:center;background-color:white;'});
	//Creacion del formulario
	var tablaForm = new dojox.layout.TableContainer({cols:1,labelWidth:'140',style:'text-align:left;'});
	...
	divCamposD.addChild(botonGrabar);
	...
	divCamposD.addChild(botonCancelar);
	//Creacion del dialogo
	...
	if(gridMaster.selection.getSelected()!=''){
		var items = gridMaster.selection.getSelected();
		console.log(gridMaster.store.getValue(items[0],columnasM[0]));
		//Aca el id de usuario que se setea al combobox tiene que estar en minuscula ya que asi vienen los id de ldap 
		dijit.byId(camposFormD[0]).attr('value',gridMaster.store.getValue(items[0],columnasM[0]).toLowerCase());
	}
	//Se muestra el dialogo
	dialogD.show();
}
...
function CopiarMaster(){ 
	//Creacion del contenedor
	var divCamposM = new dijit.layout.BorderContainer({style:'width:350px;background-color:white;'});
	//Creacion del formulario
	var tablaForm = new dojox.layout.TableContainer({cols:1,labelWidth:'140',style:'text-align:left;'});
	for(i=0;i<camposFormM.length;i++){
		tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[i],type:typeFormM[i],label:labelFormM[i],type:tiposM[i],trim:true,promptMessage:mensajesFormM[i],regExp:regexpFormM[i],required:requeridoFormM[i]}));}
...
function Eliminar(urlValue,tipo){
	...
	dojo.xhrGet({url:urlValue,handleAs:"json",load:function(response){
		if(response.errcode == 0){
			...
			if(tipo=='master'){BuscarMaster();}else{BuscarMaster();BuscarDetail();}
			procesando.hide();
...
function PreContrasenaMaster(){
	//Creacion del contenedor
	var divCamposM = new dijit.layout.BorderContainer({style:'text-align:center;'});
...
function BajadaExcelDetail(){
	var itemsM = gridMaster.selection.getSelected();
	var urlValue ='dbi/bajaExcel.php?sid='+gvarSID+'&dmlid='+dmlBuscarDetail;
	if (itemsM.length > 0) {
		urlValue = urlValue +'&'+columnasD[0]+'='+itemsM[0][columnasM[0]];
	}
	document.location = urlValue;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
