<?php
   ...
   /************************************************* 
    PROCEDURE VerAdjuntoComprobanteSP
    Obtiene el adjunto de un comprobante sin presupuesto
    ...
    *************************************************/
	...
	if ($modoLogin == 'SSO') {
		...
		$SSO_REROUTE = "ssoreroute.php?p_redirect_url=VerAdjuntoComprobanteSP.php?id=".$cbteId;
			...
			if ( $errors['errcode'] == "" ) {	//conexión a la DB ok.	
				...
				if ( $infoSesion['sid'] == "" || $infoSesion['sid'] == " " ) {
					...
					$dml_query = "SELECT NOMBRE,TIPO,TAMA,CONTENIDO FROM CMN_ARCHIVOS 
								   WHERE ARCHIVO_ID = ( SELECT csp.ARCHIVO_ID FROM COMPROBANTES_SP csp
													     WHERE csp.COMPROBANTES_SP_ID = $cbteId )";
					...
					if ( $errors['errcode'] == "" ) {
						...
						if (($result['NOMBRE'] == '') || ($result['NOMBRE'] == ' ')) {
							$errors['errcode'] = "DB-1";
							$errors['errmsg'] = "No se pudo obtener el adjunto del comprobante indicado";
							$errors['debugmsg'] = "DB"; 						
						} else {
							$size = $result['TAMA'];
							$tipo = $result['TIPO'];
