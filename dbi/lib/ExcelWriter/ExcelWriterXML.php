...
@param string $function The name of the function that was called
	 ...
	 */
	function addError($function, $message){
		$tmp = array(
			'function'	=> $function,
			'message'	=> $message,
		);
	...
	function updateSheet($sheetUpd){
		$sheetUpdID = $sheetUpd->getID();
		$this->sheets[$sheetUpdID] = $sheetUpd;
	}
	
	function updateStyle($styleUpd){
		$styleUpdID = $styleUpd->getID();
		$this->styles[$styleUpdID] = $styleUpd;
	}
     ...
     */
	function writeData($target = null,$codificacion = 'UTF-8'){
		$docTitle = '';
		$docSubject = '';
		$docAuthor = '';
		$docCreated = '';
		...
		$xml = '<?xml version="1.0" encoding="'.$codificacion.'"?>'."\r";
     ...
     */
	function mysqlTableDump($host,$username,$password,$db,$table,$alternateName = null){
		...
		if ($this->checkStyleID('db_header')){
			$sHeader = $this->addStyle('db_header');
			$sHeader->fontBold();
			$sHeader->fontFamily('Swiss');
			$sHeader->fontColor('0000FF');
		}
		if ($this->checkStyleID('db_datetime')){
			$hDateTime = $this->addStyle('db_datetime');
			$hDateTime->numberFormatDateTime();
		}
		if ($this->checkStyleID('db_date')){
			$hDate = $this->addStyle('db_date');
			$hDate->numberFormatDate();
		}
		if ($this->checkStyleID('db_time')){
			$hTime = $this->addStyle('db_time');
