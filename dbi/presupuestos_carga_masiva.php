<?php
	...
	if($_FILES['userfile']['size'] > 0)
	{
		...
		$columnas = 'HEADER_ID,USR_ALTA,RESPONSABILIDAD_ID,ESQUEMA,SOCIEDAD_ID,TIPO_CARGA,AREA_ID,TIPO_PRESUPUESTO_ID,DIVISION_ID,NEGOCIO_ID,ANIO,MES,IMPORTE,VOLUMEN_VENTAS,PRECIO_HTL,OBSERVACIONES,SUBREGION_ID,IMPORTE_SUBP,VOLUMEN_VENTAS_SUBP,PRECIO_HTL_SUBP,OBSERVACIONES_SUBP';
		$table_name = 'PRESUPUESTOS_CM_TEMP';
		$accionSql = 'I';
		...
		for($i=2;$i<=$cantFilas;$i++){
			...
			$valores[7] = str_replace(',','.',$data->val($i,'G'));
  			$valores[8] = str_replace(',','.',$data->val($i,'H'));
			$valores[9] = str_replace(',','.',$data->val($i,'I'));
            $valores[10] = $data->val($i,'J');
			$valores[11] = $data->val($i,'K');
			$valores[12] = str_replace(',','.',$data->val($i,'L'));
			$valores[13] = str_replace(',','.',$data->val($i,'M'));
			$valores[14] = str_replace(',','.',$data->val($i,'N'));
			$valores[15] = $data->val($i,'O');
