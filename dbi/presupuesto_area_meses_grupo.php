<?php
    ...
    *************************************************/
				...
				if(is_null($accionesPerfil['A'.$accionid])) {
					...
					$dml = "SELECT PRESUPUESTO_ID,MES,IMPORTE,VOLUMEN_VENTAS,PRECIO_HTL
							  FROM PRESUPUESTOS WHERE AREA_ID = $area_id 
								AND trim(DIVISION_ID) = 998
								AND NEGOCIO_ID = $negocio_id
								AND trim(MARCA_ID) = trim('$marca_no_aplica')
								AND ANIO = '$anio'
	...
	if ($errors['errcode'] == "") {
		...
		echo utf8_encode('{errcode:"' . $errors['errcode'] . '", errmsg:"' . $errors['errmsg'] . '", srvmsg:"' . $errors['srvmsg'] . '", debugmsg:"' . $errors['debugmsg'] . '", dml:"' . $dml . '"}');
	}
?>
