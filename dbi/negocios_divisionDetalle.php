			...
			if ( $errors['errcode'] == "" ) {  		
				...
				if (($dmls['dmlPropuestas'] == "")||($dmls['dmlMarcasPropuestas'] == "")) {
					...
					if ( $errors['errcode'] == "" ) {
						...
						$varSalida = "identifier:\"division_id\",";
						$varSalida = $varSalida . "label:\"division_id\",";
						...
						while ($result = $con->getArrayResultadoQuery($stmt)) {
							...
							if ( $errors['errcode'] == "" ){
                                ...
                                while ($result2 = $con->getArrayResultadoQuery($stmt2)) {
                                   ...
                                   $marcass = $marcass . $result2['MARCA_ABREV'];
                                }
                            ...
                            $varSalida = $varSalida . '{"neg_div_marca_id":"' . $result['NEG_DIV_MARCA_ID'] . '","division_id":"' . $result['MARCAS'] . '","division_desc":"' . $result['DIVISIONES'] . '","status":"' . $result['STATUS'] . '","exclusivo":"' . $result['EXCLUSIVO'] . '","negocio_id":"' . $result['NEGOCIO_ID'] . '","negocio_desc":"' . $result['NEGOCIO_DESC'] . '","division":"' . $result['DIVISION_ID'] . '","marca_mostrar":"' . $result['MARCA_MOSTRAR'] .  '","neg_div_marca_id":"' . $result['NEG_DIV_MARCA_ID'] .'"}';
						}
