							...
							if($presupuesto_id == ""){
								$dml = "SELECT NULL AS SUBPRESUPUESTO_ID,
											   ...
											   (select nvl(sum(pp.importe),0) 
												  from propuestas pr,propuestas_productos pp, mgc.productos vp, mgc.productos_estadisticos pe
												 where pr.AREA_ID = '$area_id'
												   and pr.SUBREGION_ID = sr.SUB_REGION_ID
												   and pr.TIPO_PRESUPUESTO_ID = '$tipo_presupuesto_id'
												   and pr.ANIO = '$anio'
												   and pr.MES = '$mes'
												   and trim(pr.DIVISION_ID) = trim('$division_id')
												   and pr.ESQUEMA = '$esquema'
												   and pr.SOCIEDAD_ID = '$sociedad_id'
												   and pr.estado in ('EA','EW','AW')
												   and pp.PROPUESTA_ID = pr.PROPUESTA_ID
												   and pp.PRODUCTO_ID = vp.PRODUCTO_ID
												   and pp.ESQUEMA = vp.ESQUEMA
												   and pp.SOCIEDAD_ID = vp.SOCIEDAD_ID
												   and vp.PRODUCTO_ESTADISTICO_ID = pe.PRODUCTO_ESTADISTICO_ID
												   and vp.ESQUEMA = pe.ESQUEMA
												   and trim(pe.MARCA_ID) = trim('$marca_id')) as PROP,
											   0 as DISP
										  ...
										  WHERE sr.ESQUEMA = '$esquema'
											...
											AND sr.SUB_REGION_ID in (SELECT SUBREGION_ID FROM SUBREGIONES_HABILITADAS 
																	  WHERE ESQUEMA = sr.ESQUEMA
																	    AND SOCIEDAD_ID = sr.SOCIEDAD_ID AND HABILITADA = 1)
											AND sr.SUB_REGION_ID IN (SELECT pr.SUBREGION_ID from propuestas pr,propuestas_productos pp, mgc.productos vp, mgc.productos_estadisticos pe
																						   where pr.AREA_ID = '$area_id'
																							 and pr.SUBREGION_ID = sr.SUB_REGION_ID
																							 and pr.TIPO_PRESUPUESTO_ID = '$tipo_presupuesto_id'
																							 and pr.ANIO = '$anio'
																							 ...
																							 and vp.PRODUCTO_ESTADISTICO_ID = pe.PRODUCTO_ESTADISTICO_ID
																							 and vp.ESQUEMA = pe.ESQUEMA
																							 and trim(pe.MARCA_ID) = trim('$marca_id'))";
							} else {
								$dml = "SELECT s.SUBPRESUPUESTO_ID,
											   ...
											   (select nvl(sum(pp.importe),0) 
												 ...
												 where pr.AREA_ID = p.AREA_ID
												   ...
												   and pr.ANIO = p.ANIO
												   and pr.MES = p.MES
												   and trim(pr.DIVISION_ID) = trim(p.DIVISION_ID)
												   ...
												   and vp.PRODUCTO_ESTADISTICO_ID = pe.PRODUCTO_ESTADISTICO_ID
												   and vp.ESQUEMA = pe.ESQUEMA
												   and trim(pe.MARCA_ID) = trim(p.MARCA_ID)) as PROP,
											   (s.importe - (select nvl(sum(pp.importe),0) 
												 ...
												 where pr.AREA_ID = p.AREA_ID
												   ...
												   and vp.PRODUCTO_ESTADISTICO_ID = pe.PRODUCTO_ESTADISTICO_ID
												   and vp.ESQUEMA = pe.ESQUEMA
												   and trim(pe.MARCA_ID) = trim(p.MARCA_ID))) as DISP
