<?php
	...
	if($_FILES['userfile']['size'] > 0)
	{
		...
		$columnas = 'HEADER_ID,USR_ALTA,RESPONSABILIDAD_ID,ESQUEMA,SOCIEDAD_ID,TIPO_CARGA,BULTOS,ANIO,MES,TIPO_CARGA_PROPUESTA,TIPO_PRESUPUESTO_ID,TIPO_DESCUENTO_ID,CLIENTE_ID,PARTICIPACION_CLIENTE,DESCRIPCION_PROPUESTA_ID,EMPRESA_COMPETENCIA_ID,DISTRIBUCION_FISICA_COMP,OBSERVACIONES,OBJETIVO_PROPUESTA_ID,UNIDADES_ACCION,OP_REALIZAR,PRODUCTO_ID,IMPORTE';
		$table_name = 'PROPUESTAS_CM_TEMP';
		$accionSql = 'I';
		...
		for($i=2;$i<=$cantFilas;$i++){
			...
			$valores[12] = $data->val($i,'L');
			$valores[13] = $data->val($i,'N');
			$valores[14] = str_replace(',','.',$data->val($i,'O'));
    				...
    				if ( $errors['errcode'] == "" ) {  		
						...
						$i = 0;			
    								foreach($datos as $key => $data){
    									...
    									$data[1] = $header_id . ',' . $infoSesion['usuarioId'] . ',' . $infoSesion['respId'] . ',' . $infoSesion['esquema'] . ',' . $infoSesion['sociedad_id'] . ',' . $tipo_carga . ',' . "0" . ',' . $anio . ',' . $mes . ',' . $data[1];
