<?php
	...
	$dmlidSubregiones = '11507';
    $dmlidPresups = '11508';
    $dmlidSubregsPresups = '11513';
	$dmlidSubregsProps = '11514';
	...
	if ( $errors['errcode'] == "" ) {
		...
		if (is_null($infoSesion['usuarioId'])) {      //si usuario es nulo es que la sesión no existe
			...
			if (($anio == $anioActual && $mes < $mesActual) || ($anio < $anioActual)) {
				$dmlidSubregsPresups = '11511';
				$dmlidSubregsProps = '11512';
			}
			...
			if ( $errors['errcode'] == "" ) {  		
				...
				if (($dmls['dmlSubregiones'] == "")||($dmls['dmlPresups'] == "")||($dmls['dmlSubregsPresups'] == "")||($dmls['dmlSubregsProps'] == "")) {
					...
					foreach($_POST as $nombre_campo => $valor){ 
						if (($nombre_campo != 'marca_id') && ($nombre_campo != 'presupuesto_id')) {
                    ...
                    $sheet->writeString(1,1,'Marca','Cabecera');
			...
			echo '<br><br>'.$dmls['dmlPresups'];
					...
					if ( $errors['errcode'] == "" ) {
						...
						while ($result = $con->getArrayResultadoQuery($stmt)) {
							...
							if (($anio == $anioActual && $mes < $mesActual) || ($anio < $anioActual)) {
								$dmlSubregsProps = str_replace("#marca_id#", $marca_id, $dmls['dmlSubregsProps']);
						echo '<br><br>'.$dmlSubregsProps;
								...
								if ( $errors['errcode'] == "" ){
									...
									$dmlsSubregsPresups = str_replace("#marca_id#", $marca_id, $dmlsSubregsPresups);
							echo '<br><br>'.$dmlsSubregsPresups;
									...
									if ( $errors['errcode'] == "" ){
										...
										$sheet->writeString($t2,1,$result['MARCA_ABREV'],'Cabecera');
