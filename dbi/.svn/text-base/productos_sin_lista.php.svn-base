<?php
	...
	if($_FILES['userfile']['size'] > 0)
	{
		...
		$columnas = 'HEADER_ID,USR_ALTA,RESPONSABILIDAD_ID,ESQUEMA,SOCIEDAD_ID,ANIO,MES,CLIENTE_ID,PRODUCTO_ID,PRODUCTO_REEMPLAZO_ID';
		$table_name = 'PROVI_ERR_PRODS_CM_TEMP';
		$accionSql = 'I';
    				...
    				if ( $errors['errcode'] == "" ) {  		
						...
						$i = 0;			
    								foreach($datos as $key => $data){
    									...
    									$data[1] = $header_id . ',' . $infoSesion['usuarioId'] . ',' . $infoSesion['respId'] . ',' . $infoSesion['esquema'] . ',' . $infoSesion['sociedad_id'] . ',' . $data[1];
