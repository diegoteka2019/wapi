<?php
	...
	if($_FILES['userfile']['size'] > 0)
	{
		...
		$columnas = 'HEADER_ID,USR_ALTA,RESPONSABILIDAD_ID,ESQUEMA,SOCIEDAD_ID,TIPO_CARGA,SUBREGION_ID,CLIENTE_ID,OBSERVACIONES,VIGENCIA_DESDE,VIGENCIA_HASTA,TIPO_PLAN_ESPECIAL_ID,TIPO_OPERACION_ID,TIPO_LIMITE_CREDITO_ID,CONDICION_PAGO_ID,DEPOSITO_DESTINO_ID,EXPIRACION_COMODATO,DIVISION_ID,PRODUCTO_ID,CANTIDAD_PRESTAMO_COMODATO,CANTIDAD_SIN_CARGO,CANTIDAD_VENTA';

		$table_name = 'PLANES_ESPECIALES_CM_TEMP';
		$accionSql = 'I';
		...
		for($i=2;$i<=$cantFilas;$i++){
			...
			$valores[13] = $data->val($i,'M');
			$valores[14] = str_replace(',','.',$data->val($i,'N'));
			$valores[15] = str_replace(',','.',$data->val($i,'O'));
			$valores[16] = str_replace(',','.',$data->val($i,'P'));
