<?php
   ...
   /************************************************* 
    ...
    Obtiene el adjunto de una acreditacion
		acreditacion_id:	Identificador de la acreditacion

	21/07/2010	Esteban Lopez	Creacion version inicial
    *************************************************/
	...
	$dmlidBuscarAdjunto = '11814';
	$cbteId = $_POST['acreditacion_id'] . $_GET['acreditacion_id'];
	//Interfaz PHP-DBI
	$result = null;
	$con = null;
	$stmt = null;
				...
				if ($dmls['dmlBuscarAdjunto'] == "") {
					...
					if ( $errors['errcode'] == "" ) {
						...
						if (($result['NOMBRE'] == '') || ($result['NOMBRE'] == ' ')) {
							$errors['errcode'] = "DB-1";
							$errors['errmsg'] = "No se pudo obtener el adjunto de la acreditacion";
							$errors['debugmsg'] = "DB"; 						
						} else {
							$size = $result['TAMA'];
							$tipo = $result['TIPO'];
