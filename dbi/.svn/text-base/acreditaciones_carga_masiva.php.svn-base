<?php
	...
	if($_FILES['userfile']['size'] > 0)
	{
		...
		$columnas = 'HEADER_ID,USR_ALTA,RESPONSABILIDAD_ID,ESQUEMA,SOCIEDAD_ID,TIPO_CARGA,PROPUESTA_ID,IMPORTE_TOTAL,DIVISION_ID,IMPORTE_PENDIENTE,OBSERVACIONES,DEPOSITO_ID,IMPORTE,TIPO_COMPROBANTE_ID,SEQ_ACRED';
		$table_name = 'ACREDITACIONES_CM_TEMP';
		$accionSql = 'I';
		...
		for($i=2;$i<=$cantFilas;$i++){
			...
			$valores[2] = str_replace(',','.',$data->val($i,'B'));
			$valores[3] = $data->val($i,'C');
			$valores[4] = str_replace(',','.',$data->val($i,'D'));
			$valores[5] = $data->val($i,'E');
			$valores[6] = $data->val($i,'F');
			$valores[7] = str_replace(',','.',$data->val($i,'G'));
			$valores[9] = $data->val($i,'H');
			$valores[10] = $data->val($i,'I');
						...
						$i = 0;			
    								...
    								if($booErrorFillData == true){
                    					...
                    					if($ticket != false){
											...
											$varFilas = 'Filas Insertadas:"'.$cantFilas.'"';
                    					}
    								}
    							}
    						}
	...
	if ($varSalida == "")	
		...
		echo utf8_encode('<textarea>{' . $varFilas . '}</textarea>');
