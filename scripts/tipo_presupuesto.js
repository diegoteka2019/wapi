//TITULOS
var titulos =  new Array("Tipos de Presupuesto");
//DMLs
var dmlBuscarMaster = "10105";
var dmlnuevoM = "10101";
var dmleditarM = "10102";
var dmlbuscarUnicoM = "10104"; 
var dmleliminarM = "10103";
var dmlAuditoriaMaster = "10106";
var dmlgrabarM = "";
//COLUMNAS
var columnasM = new Array("tipo_presupuesto_id","descripcion");
var tiposM = new Array("text","text");
//DESCRIPCIONES
var descripcionesM = new Array("C&oacute;digo","Descripci&oacute;n");
//PRIMARY KEY
var clavesM = new Array("tipo_presupuesto_id");
//FILTROS
...
//FORMULARIO
var camposFormM = new Array('tipo_presupuesto_id','descripcion');
var labelFormM = new Array('C&oacute;digo: ','Descripci&oacute;n: ');
var requeridoFormM = new Array(true,true);
var regexpFormM = new Array(".*","[a-zA-Z0-9Ò—ø?°! ,.()-_]+");
var mensajesFormM = new Array('Ingrese el c&oacutedigo',"Ingrese la descripci&oacute;n");
...
function NuevoMaster(){
	...
	dijit.byId('tipo_presupuesto_id').attr('disabled',true);
	dijit.byId('tipo_presupuesto_id').attr('required',false);
...
function EditarMaster(){
	...
	for(i=0;i<columnasM.length;i++){
		tablaForm.addChild(new dijit.form.ValidationTextBox({id:columnasM[i],label:labelFormM[i],trim:true,maxLength:'40',promptMessage:mensajesFormM[i],regExp:regexpFormM[i],required:requeridoFormM[i]}));}
	divCamposM.attr('content',tablaForm);
	//Creacion de botones
	var botonGrabar = new dijit.form.Button({label:'Grabar',type:'button',iconClass:'dijitEditorIcon dijitEditorIconSave',onClick:function(){GrabarMaster();}});
	dojo.place(botonGrabar.domNode,divCamposM.domNode,'last');
...
function GrabarMaster(){
	...
	procesando.show();
	//reemplazo caractÈr &
	dijit.byId(camposFormM[1]).attr('value',dijit.byId(camposFormM[1]).attr('value').replace('&','%26'));
	dijit.byId(camposFormM[1]).attr('value',unescape( encodeURIComponent( dijit.byId(camposFormM[1]).attr('value') ) ) );
