...
//DMLs
var dmlBuscarMaster = '11504';
var dmlnuevoM = '';
var dmleditarM = '11502';
...
function BuscarMaster(){
	...
	if(filtroMaster.length > 0){
		for(i=0;i<filtroMaster.length;i++){
			...
			if(i!=3&&i!=4){dojo.byId('mostrarFiltros').innerHTML += '<b>' + labelFiltroMaster[i] + '</b>' + dijit.byId('filtro'+i).item.detalle[0] + ' ';}
	...
	var CargarGrillas = function(items,request){
		...
		for(i=0;i<items.length;i++){
			...
			var cabecera = dojo.create('div',{innerHTML:'<table style="width:90%;font-weight:bold;"><tr><td width="20%">'+items[i]['marca_abrev'][0]+'</td><td width="20%">$ Presupuesto: '+items[i]['importe'][0]+'</td><td width="20%">Vol. Total: '+items[i]['volumen_ventas'][0]+'</td><td width="20%">$/Htl: '+items[i]['precio_htl'][0]+'</td><td width="20%">$ Pendiente: '+items[i]['pendiente'][0]+'</td></tr></table>'});
			dojo.attr(cabecera,'class','dijitToolbar');
			dojo.place(cabecera,dojo.byId('gridContainer'),'last');
			//Creacion de grillas dinamicas
			var urlValue2 ='dbi/presupuesto_region_marcaSubregiones.php?sid='+gvarSID+'&marca_id='+items[i]['marca_id'][0]+'&presupuesto_id='+items[i]['presupuesto_id'][0];//+'&area_id='+dijit.byId('filtro0').attr('value');
        	if(filtroMaster.length > 0){
...
function EditarMaster(){
	...
	if(dijit.byId(camposFormM[0])){dijit.byId(camposFormM[0]).destroy();}tablaForm.addChild(new dijit.form.TextBox({id:camposFormM[0],readOnly:true,style:'width:130px;',label:labelFormM[0]}));
	if(dijit.byId(camposFormM[1])){dijit.byId(camposFormM[1]).destroy();}tablaForm.addChild(new dijit.form.TextBox({id:camposFormM[1],readOnly:true,style:'width:130px;',label:labelFormM[1]}));
	if(dijit.byId(camposFormM[2])){dijit.byId(camposFormM[2]).destroy();}tablaForm.addChild(new dijit.form.TextBox({id:camposFormM[2],readOnly:true,style:'width:130px;',label:labelFormM[2]}));
	if(dijit.byId(camposFormM[3])){dijit.byId(camposFormM[3]).destroy();}tablaForm.addChild(new dijit.form.FilteringSelect({id:camposFormM[3],style:'width:130px;',value:"1",label:labelFormM[3],searchAttr:searchAttrFormM[3],promptMessage:mensajesFormM[3],required:requeridoFormM[3],store:new dojo.data.ItemFileReadStore({data: cargaStore}),onChange:function test(){if(this.attr('value')==1){dijit.byId('importeNuevo').attr('readOnly',false);dijit.byId('precioLitroNuevo').attr('value','0');dijit.byId('precioLitroNuevo').attr('readOnly',true);}else{dijit.byId('importeNuevo').attr('readOnly',true);dijit.byId('importeNuevo').attr('value','0');dijit.byId('precioLitroNuevo').attr('readOnly',false);}}}));
	if(dijit.byId(camposFormM[4])){dijit.byId(camposFormM[4]).destroy();}tablaForm.addChild(new dijit.form.FilteringSelect({id:camposFormM[4],style:'width:130px;',value:"1",label:labelFormM[4],searchAttr:searchAttrFormM[4],promptMessage:mensajesFormM[4],required:requeridoFormM[4],store:new dojo.data.ItemFileReadStore({data: ajusteStore})}));
...
function BajadaExcelMaster() {
	var urlValue ='dbi/presupuesto_region_excel.php?sid='+gvarSID;
    if(filtroMaster.length > 0){
		for(i=0;i<filtroMaster.length;i++){
			urlValue +='&'+filtroMaster[i]+'='+dijit.byId('filtro'+i).attr('value');}
	}
