...
var dmlgrabarM = '';
//COLUMNAS
var columnasM = new Array('propuesta_id','marcas','vigencia_desde','vigencia_hasta','descripcion','bultos','importe','solicitado','estado','tipo_carga');
var columnasE = new Array('propuesta_id','marcas','vigencia_desde','vigencia_hasta','descripcion','bultos','importe','solicitado','estado','seleccionado','tipo_carga');
...
//FILTROS
var filtroMaster = new Array('propuesta_id','division_id','area_id','subregion_id','tipo_presupuesto_id','mes','anio','estado');
var labelFiltroMaster = new Array('Propuesta: ','Divisi&oacute;n: ','Area: ','Sub Regi&oacute;n: ','Tipo: ','Mes: ','A&ntilde;o: ','Estado: ');
var mensajeFiltroMaster = new Array('Seleccione la Propuesta','Seleccione la Divisi&oacute;n','Seleccione el Area','Seleccione la Sub Regi&oacute;n','Seleccione el Tipo de Presupuesto','Seleccione el Mes','Seleccione el A&ntilde;o','Seleccione el Estado de la Propuesta');
...
var tipoFiltroMaster = new Array('ValidationTextBox','FilteringSelect','FilteringSelect','FilteringSelect','FilteringSelect','FilteringSelect','ValidationTextBox','ValidationTextBox');
...
function ArmadoFiltroYAuditoria(){
	if(dijit.byId('filtrMaster')){
		...
		AutoCompletar(dijit.byId('filtro2'));
		...
		dijit.byId('filtro7').attr('regExp',"[A-Z]+");
		...
		dijit.byId('filtro0').onChange= function(value){
			if(value!=''){
				...
				dijit.byId('filtro7').attr('disabled',true);dijit.byId('filtro7').attr('required',false);}
			else{dijit.byId('filtro1').attr('disabled',false);dijit.byId('filtro1').attr('required',false);
				dijit.byId('filtro2').attr('disabled',false);dijit.byId('filtro2').attr('required',true);AutoCompletar(dijit.byId('filtro2'));
				dijit.byId('filtro3').attr('disabled',false);dijit.byId('filtro3').attr('required',false);
				dijit.byId('filtro4').attr('disabled',false);dijit.byId('filtro4').attr('required',false);
				dijit.byId('filtro5').attr('disabled',false);dijit.byId('filtro5').attr('required',true);
		...
		dijit.byId('filtro3').attr('disabled',false);dijit.byId('filtro3').attr('required',false);
		dijit.byId('filtro3').store.url += '&area_id='+dijit.byId('filtro2').attr('value');
		//Borrado de subregiones al cambiar el area
		dijit.byId('filtro2').onChange= function(value){dijit.byId('filtro3').attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFiltroMaster[3]+'&area_id='+value}));dijit.byId('filtro3').store.fetch();dijit.byId('filtro3').attr('value',null);AutoCompletar(dijit.byId('filtro3'));}
...
function NuevoMaster(){
	...
	tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[5],maxLength:'40',value:'Bultos',style:'width:130px;',label:labelFormM[5],readOnly:true,required:requeridoFormM[5]}));
	tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[25],maxLength:'40',style:'width:130px;',label:labelFormM[25],required:requeridoFormM[25]}));
...
function CargarClientes(){
	...
	var ActualizarMultiselect = function(items,request){
		for(i=0;i<items.length;i++){
			...
			if(items.length > 1){
			dijit.byId('contrato').attr('readOnly',true);}
			else{dijit.byId('contrato').attr('readOnly',false);}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
function CargarProductosContrato(value){
	...
	//dmlid 11732 (division_id, marca_id, producto_id, producto_desc, contrato_id)
	var productosStore = new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=11732&contrato_id='+value+'&division_id='+value+'&marca_id='+value+'&p='+value})
	...
	productosStore.fetch({query:{},onComplete:ActualizarGrilla});
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
function CargarClientes1(){
	...
	var ActualizarMultiselect = function(items,request){
		...
		dijit.byId('contrato').attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=11730&division_id='+dijit.byId(camposFormM[0]).attr('value')+'&distribuidor_id='+dijit.byId('clientes').attr('value')}));
		dijit.byId('contrato').store.fetch();
		dijit.byId('contrato').attr('value',null);
		dijit.byId("dialogClientes").hide();
	}
...
function EditarMaster(){
	...
	var LlenarParametros = function(items,request){
		...
		}else{dijit.byId('marc').attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=11731&division_id='+dijit.byId('division_id').attr('value')+'&contrato_id='+items[0]['contrato_id'][0]}));
		}
		dijit.byId('divCampos').attr('content',tablaForm);
		tablaForm.startup();
		//Muestro el dialogo
...
function CopiarMaster(){
	...
	var LlenarParametros = function(items,request){
		...
		if(dijit.byId(camposFormM[19])){dijit.byId(camposFormM[19]).destroy();}tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[19],value:items[0]['observaciones'][0],maxLength:'40',required:requeridoFormM[19],regExp:regexpFormM[1],style:'width:130px;height:58px;',label:labelFormM[19]}));
...
function CopiarMasterEA(){
	...
	var LlenarParametros = function(items,request){
		...
		tablaForm.addChild(new dijit.form.TextBox({value:items[0]['contrato_id'][0],style:'width:130px;',label:'Contrato: ',readOnly:true}));
		if(dijit.byId('canales_mkt')){dijit.byId('canales_mkt').destroy();}tablaForm.addChild(new dojox.form.CheckedMultiSelect({id:'canales_mkt',readOnly:true,style:'width:130px;height:58px;',label:labelFormM[18],multiple:true,store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=11619&propuesta_id='+registroSeleccionado[0][columnasM[0]][0]})}));
		if(dijit.byId(camposFormM[19])){dijit.byId(camposFormM[19]).destroy();}tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[19],value:items[0]['observaciones'][0],maxLength:'40',required:requeridoFormM[19],style:'width:130px;height:58px;',regExp:regexpFormM[1],label:labelFormM[19]}));
...
function GrabarMasterEditar(){
	...
	var CompletarJSON = function(items,request){
		...
		if(ids==''){dijit.showTooltip('Ingrese al menos un producto',dijit.byId('gridProductos').domNode,'above');return;}
		if(registroSeleccionado[0]['estado'][0]=='AW'){if(dojo.byId('divTotalImporte').innerHTML>registroSeleccionado[0]['importe'][0]){dijit.showTooltip('El total de los importes no debe superar al monto original',dijit.byId('gridProductos').domNode,'above');return;}}
		dijit.byId('dialogMaster').hide();
		procesando.show();
		urlValue+='&productos='+ids+'&bultos_productos='+bultos+'&importes_productos='+importes+'&porcentajes='+porcentajes;
		//Grabación de datos en la base de datos
...
function CancelarMaster(){
	...
	if(dijit.byId('clientes')){dijit.byId('clientes').destroy();}
