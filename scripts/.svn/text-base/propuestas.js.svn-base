...
var columnasD = new Array('producto_id','producto_desc','bultos','importe','porcentaje');
...
var descripcionesD = new Array('Id','Producto','Bultos','Importe','Porcenaje');
...
//FILTROS
var filtroMaster = new Array('division_id','area_id','subregion_id','cliente_id','tipo_presupuesto_id','mes','anio','estado');
var labelFiltroMaster = new Array('Divisi&oacute;n: ','Area: ','Sub Regi&oacute;n: ','Distribuidor: ','Tipo: ','Mes: ','A&ntilde;o: ','Estado: ');
var mensajeFiltroMaster = new Array('Seleccione la Divisi&oacute;n','Seleccione el Area','Seleccione la Sub Regi&oacute;n','Seleccione el Distribuidor','Seleccione el Tipo de Presupuesto','Seleccione la Vigencia Desde','Seleccione la Vigencia Hasta','Seleccione el Estado de la Propuesta');
var requeridoFiltroMaster = new Array(false,false,false,false,false,true,true,false);
var searchAttrFiltroMaster = new Array('detalle','detalle','detalle','detalle','detalle','detalle','','');
var tipoFiltroMaster = new Array('FilteringSelect','FilteringSelect','FilteringSelect','FilteringSelect','FilteringSelect','FilteringSelect','ValidationTextBox','ValidationTextBox');
var selectFiltroMaster = new Array('11408','11406','11606','11633','11407','','','');
//FORMULARIO
...
var layoutDialog = [
	...
	{width: "105px", field: columnasD[3], name: descripcionesD[3],editable:true,formatter:CalcularTotal},
	{width: "10px", field: columnasD[4], name: descripcionesD[4],hidden:true}];
...
function DojoAddOnLoad(){
	if(dijit.byId('buscarMaster')){
        ...
        dijit.byId('toolbarMaster').addChild(new dijit.form.Button({id:'verDetalleMaster',label:'Ver Detalle',showLabel:false,iconClass:'verDetalleIcon',disabled:true,onClick:function(){VerDetalleMaster();}}));    
    }
...
function ArmadoFiltroYAuditoria(){
	if(dijit.byId('filtrMaster')){
		...
		AutoCompletar(dijit.byId('filtro4'));
		...
		dijit.byId('filtro6').attr('value',dojo.date.locale.format(new Date(),{datePattern:'yyyy',selector:'date'}));
...
function NuevoMaster(){
	...
	tablaForm.addChild(new dijit.form.FilteringSelect({id:camposFormM[0],style:'width:130px;',label:labelFormM[0],searchAttr:searchAttrFormM[0],promptMessage:mensajesFormM[0],required:requeridoFormM[0],store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[0]}),onChange:function(value){dijit.byId(camposFormM[12]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[12]+'&division_id='+value}));dijit.byId(camposFormM[12]).store.fetch();dijit.byId(camposFormM[12]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[12]));dijit.byId(camposFormM[13]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[13]+'&division_id='+value}));dijit.byId(camposFormM[13]).store.fetch();dijit.byId(camposFormM[13]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[13]));dijit.byId(camposFormM[14]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[14]+'&division_id='+value}));dijit.byId(camposFormM[14]).store.fetch();dijit.byId(camposFormM[14]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[14]));dijit.byId(camposFormM[22]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[22]+'&division_id='+value}));dijit.byId(camposFormM[22]).store.fetch();dijit.byId(camposFormM[22]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[22]));dijit.byId(camposFormM[23]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[23]+'&division_id='+value+'&area_id='+dijit.byId('area_id').attr('value')+'&subregion_id='+dijit.byId('subregion_id').attr('value')}));dijit.byId(camposFormM[23]).store.fetch();dijit.byId(camposFormM[23]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[23]));dijit.byId('marc').attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=11409&division_id='+value}));ObtenerNoAplica();}}));
	...
	tablaForm.addChild(new dijit.form.FilteringSelect({id:camposFormM[1],value:(dijit.byId('filtro1').attr('value'))?dijit.byId('filtro1').attr('value'):null,style:'width:130px;',label:labelFormM[1],searchAttr:searchAttrFormM[1],promptMessage:mensajesFormM[1],required:requeridoFormM[1],store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[1]}),onChange:function(value){CambiarCliente();dijit.byId(camposFormM[2]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[2]+'&area_id='+value}));dijit.byId(camposFormM[2]).store.fetch();dijit.byId(camposFormM[2]).attr('value',(dijit.byId('filtro2').attr('value'))?dijit.byId('filtro2').attr('value'):null);AutoCompletar(dijit.byId(camposFormM[2]));dijit.byId(camposFormM[23]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[23]+'&division_id='+dijit.byId('division_id').attr('value')+'&area_id='+value+'&subregion_id='+dijit.byId('subregion_id').attr('value')}));dijit.byId(camposFormM[23]).store.fetch();dijit.byId(camposFormM[23]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[23]));}}));
	AutoCompletar(dijit.byId(camposFormM[1]));
	tablaForm.addChild(new dijit.form.FilteringSelect({id:camposFormM[2],style:'width:130px;',label:labelFormM[2],searchAttr:searchAttrFormM[2],promptMessage:mensajesFormM[2],required:requeridoFormM[2],store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[2]+'&area_id='+dijit.byId(camposFormM[1]).attr('value')}),onChange:function(value){CambiarCliente();dijit.byId(camposFormM[23]).attr('store',new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFormM[23]+'&division_id='+dijit.byId('division_id').attr('value')+'&area_id='+dijit.byId('area_id').attr('value')+'&subregion_id='+value}));dijit.byId(camposFormM[23]).store.fetch();dijit.byId(camposFormM[23]).attr('value',null);AutoCompletar(dijit.byId(camposFormM[23]));}}));
	...
	tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[5],value:'Importe',style:'width:130px;',label:labelFormM[5],readOnly:true,required:requeridoFormM[5]}));
	...
	tablaForm.addChild(new dijit.form.SimpleTextarea({id:camposFormM[19],required:requeridoFormM[19],style:'width:130px;height:58px;',label:labelFormM[19],maxLength:'100'}));
		...
		for(i=0;i<items.length;i++){
			dijit.byId('gridProductos').store.fetchItemByIdentity({
				...
				onItem:function(item){
					if(!item){dijit.byId('gridProductos').store.newItem({producto_id:dijit.byId('gridDestinoP').store.getValue(items[i],'producto_id'), producto_desc:dijit.byId('gridDestinoP').store.getValue(items[i],'producto_desc'),bultos:'0',importe:'0',porcentaje:''});}
				}
			});
		}
		dijit.byId('gridProductos').store.save();
...
function BuscarProductos(){
	var urlValue ='dbi/q.php?sid='+gvarSID+'&dmlid=11612&division_id='+dijit.byId('division_id').attr('value')+'&marca_id='+dijit.byId('marc').attr('value')+'&producto_id='+dijit.byId('prod_id').valueNode.value+'&producto_desc='+dijit.byId('desc').attr('value');
...
function EditarMaster(){
	...
	var LlenarParametros = function(items,request){
		...
		if(dijit.byId('canales_mkt')){dijit.byId('canales_mkt').destroy();}tablaForm.addChild(new dojox.form.CheckedMultiSelect({id:'canales_mkt',readOnly:true,style:'width:130px;height:58px;',label:labelFormM[18],multiple:true,readOnly:true,store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=11619&propuesta_id='+registroSeleccionado[0][columnasM[0]][0]})}));
...
function GrabarMaster(){
	...
	for(i=0;i<requeridoFormM.length;i++){
		if(camposFormM[i]!='participacion_cli' &&camposFormM[i]!='dist_fisica_comp' &&camposFormM[i]!='bultos_afectados' &&camposFormM[i]!='op_realizar' &&camposFormM[i]!='observaciones' && camposFormM[i]!='canales_mkt' && camposFormM[i]!='clientes' && camposFormM[i]!='propuesta_id' && camposFormM[i]!='tipo_carga' && camposFormM[i]!='prorratea_calibres' && camposFormM[i]!='fecha_carga' && !dijit.byId(camposFormM[i]).isValid()){dijit.byId(camposFormM[i]).focus();return;}
	}
