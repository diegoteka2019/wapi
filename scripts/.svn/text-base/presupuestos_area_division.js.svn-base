...
//FILTROS DIVISION
var filtroMaster = new Array('area_id','tipo_presupuesto_id','division_id','anio');
var labelFiltroMaster = new Array('Area: ','Tipo: ','Division: ','A&ntilde;o: ');
var mensajeFiltroMaster = new Array('Seleccione el Area','Seleccione el Tipo de Presupuesto','Seleccione la Divisi&oacute;n','Ingrese el A&ntilde;o');
var requeridoFiltroMaster = new Array(true,true,true,true,true);
var tipoFiltroMaster = new Array('FilteringSelect','FilteringSelect','FilteringSelect','NumberSpinner');
var searchAttrFiltroMaster = new Array('detalle','detalle','detalle','','');
var selectFiltroMaster = new Array('11406','11407','11408','','');
//FILTROS GRUPOS DE NEGOCIO
var filtroMasterG = new Array('area_id','tipo_presupuesto_id','negocio_id','anio');
var labelFiltroMasterG = new Array('Area: ','Tipo: ','Negocio: ','A&ntilde;o: ');
var mensajeFiltroMasterG = new Array('Seleccione el Area','Seleccione el Tipo de Presupuesto','Seleccione el Grupo de Negocio','Ingrese el A&ntilde;o');
var requeridoFiltroMasterG = new Array(true,true,true,true);
var tipoFiltroMasterG = new Array('FilteringSelect','FilteringSelect','FilteringSelect','NumberSpinner');
var searchAttrFiltroMasterG = new Array('detalle','detalle','detalle','');
...
function ArmadoFiltroYAuditoriaDiv(){
	if(dijit.byId('filtrMaster')){
		...
		dijit.byId('filtro3').constraints.max = anioActual + 100;
		dijit.byId('filtro3').constraints.min = anioActual - 100;
		dijit.byId('filtro3').attr('value',anioActual);
		
		AutoCompletar(dijit.byId('filtro0'));
		AutoCompletar(dijit.byId('filtro1'));
		//Seteo el anio actual por defecto
		dijit.byId('filtro3').attr('value',dojo.date.locale.format(new Date(),{datePattern:'yyyy',selector:'date'}));
...
function BuscarMasterDiv(){
	...
	var urlValue ='dbi/presupuesto_area_meses_div.php?sid='+gvarSID;
	if(filtroMaster.length > 0){
		for(i=0;i<filtroMaster.length;i++){
			urlValue = urlValue +'&'+filtroMaster[i]+'='+dijit.byId('filtro'+i).attr('value');
			if(i!=3){dojo.byId('mostrarFiltros').innerHTML += '<b>' + labelFiltroMaster[i] + '</b>' + dijit.byId('filtro'+i).item.detalle[0] + ' ';}
...
function EditarMaster(){
	...
	if(anioActual>dijit.byId('filtro3').attr('value')){Mensaje('No es posible modificar el presupuesto de un a&ntilde;o vencido','advertencia','master');return;}
	if(mesActual>items[0][columnasM[1]] && anioActual==dijit.byId('filtro3').attr('value')){Mensaje('No es posible modificar el presupuesto de un mes vencido','advertencia','master');return;}
	//Creacion del formulario
	...
	var storeMails = new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=13206&area_id='+dijit.byId('filtro0').attr('value')+'&tipo_presupuesto_id='+dijit.byId('filtro1').attr('value')+'&division_id='+dijit.byId('filtro2').attr('value')+'&negocio_id=999&anio='+dijit.byId('filtro3').attr('value')+'&mes='+items[0][columnasM[1]],urlPreventCache:true})
	}else{
	var storeMails = new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=13206&area_id='+dijit.byId('filtro0').attr('value')+'&tipo_presupuesto_id='+dijit.byId('filtro1').attr('value')+'&division_id=998&negocio_id='+dijit.byId('filtro2').attr('value')+'&anio='+dijit.byId('filtro3').attr('value')+'&mes='+items[0][columnasM[1]],urlPreventCache:true})
	}
	var hayDatos = function(items,request){
		if(items.length==0){
			if(dijit.byId('mails')){dijit.byId('mails').destroy();}
			tablaForm.addChild(new dojox.form.CheckedMultiSelect({id:'mails',required:false,style:'width:130px;height:58px;',label:'Notificar a: ',multiple:true,store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=13205&area_id='+dijit.byId('filtro0').attr('value')})}));
		}
		tablaForm.startup();
...
function GrabarMaster(){
	...
	if (dijit.byId('filtro2').attr('label') == 'Division: '){
	var urlValue ='dbi/presupuesto_area_grabar_div.php?sid='+gvarSID+'&area_id='+dijit.byId('filtro0').attr('value')+'&tipo_presup='+dijit.byId('filtro1').attr('value')+'&negocio_id=999&division_id='+dijit.byId('filtro2').attr('value')+'&anio='+dijit.byId('filtro3').attr('value')+'&mes='+dijit.byId(camposFormM[0]).attr('value')+'&tipo_carga='+dijit.byId(camposFormM[1]).attr('value')+'&tipo_ajuste='+dijit.byId(camposFormM[2]).attr('value')+'&importe='+dijit.byId('importeNuevo').attr('value')+'&vol_ventas='+dijit.byId('volumenVentasNuevo').attr('value')+'&precio_htl='+dijit.byId('precioLitroNuevo').attr('value')+'&observaciones='+dijit.byId('observaciones').attr('value');
	}else{var urlValue ='dbi/presupuesto_area_grabar_div.php?sid='+gvarSID+'&area_id='+dijit.byId('filtro0').attr('value')+'&tipo_presup='+dijit.byId('filtro1').attr('value')+'&negocio_id='+dijit.byId('filtro2').attr('value')+'&division_id=998&anio='+dijit.byId('filtro3').attr('value')+'&mes='+dijit.byId(camposFormM[0]).attr('value')+'&tipo_carga='+dijit.byId(camposFormM[1]).attr('value')+'&tipo_ajuste='+dijit.byId(camposFormM[2]).attr('value')+'&importe='+dijit.byId('importeNuevo').attr('value')+'&vol_ventas='+dijit.byId('volumenVentasNuevo').attr('value')+'&precio_htl='+dijit.byId('precioLitroNuevo').attr('value')+'&observaciones='+dijit.byId('observaciones').attr('value');}
...
function ArmadoFiltroYAuditoriaGrupo(){
	if(dijit.byId('filtrMaster')){
		...
		//Creo Filtros
		var divFiltros = new dijit.layout.ContentPane({style:'width:350px;'});
		var tablaFiltrosG = new dojox.layout.TableContainer({cols:1});
		for(i=0;i<filtroMasterG.length;i++){
			if(tipoFiltroMasterG[i]=='TextBox'){tablaFiltrosG.addChild(new dijit.form.TextBox({id:'filtro'+i,label:labelFiltroMasterG[i],type:'text',promptMessageG:mensajeFiltroMasterG[i]}));}
			if(tipoFiltroMasterG[i]=='ValidationTextBox'){tablaFiltrosG.addChild(new dijit.form.ValidationTextBox({id:'filtro'+i,label:labelFiltroMasterG[i],type:'text',promptMessage:mensajeFiltroMasterG[i],required:requeridoFiltroMasterG[i]}));}
			if(tipoFiltroMasterG[i]=='DateTextBox'){tablaFiltrosG.addChild(new dijit.form.DateTextBox({id:'filtro'+i,label:labelFiltroMasterG[i],type:'text',promptMessage:mensajeFiltroMasterG[i],required:requeridoFiltroMasterG[i],onChange:function(){dijit.byId('filtro5').constraints.min = arguments[0];}}));}if(dijit.byId('filtro5')){dijit.byId('filtro5').attr('onChange',function(){dijit.byId('filtro4').constraints.max = arguments[0];})}
			if(tipoFiltroMasterG[i]=='FilteringSelect'){tablaFiltrosG.addChild(new dijit.form.FilteringSelect({id:'filtro'+i,label:labelFiltroMasterG[i],type:'text',promptMessage:mensajeFiltroMasterG[i],required:requeridoFiltroMasterG[i],searchAttr:searchAttrFiltroMasterG[i],store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid='+selectFiltroMasterG[i]})}));}
			if(tipoFiltroMasterG[i]=='NumberSpinner'){tablaFiltrosG.addChild(new dijit.form.NumberSpinner({id:'filtro'+i,label:labelFiltroMasterG[i],type:'number',promptMessage:mensajeFiltroMasterG[i],required:requeridoFiltroMaster[i],searchAttr:searchAttrFiltroMasterG[i]}));}}
		...
		divFiltros.attr('content',tablaFiltrosG);
		var botonBuscarG = new dijit.form.Button({label:'Buscar',type:'button',onClick:function(){VerificarFiltroGrupo();}},document.createElement('div'));
		dojo.place(botonBuscarG.domNode,divFiltros.domNode,'last');
		dijit.byId('filtrosMaster').attr('content',divFiltros);}	
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
function BuscarMasterGrupo(){
	//Armado de URL
	dojo.byId('mostrarFiltros').innerHTML = '&nbsp;';
	var urlValue ='dbi/presupuesto_area_meses_grupo.php?sid='+gvarSID;
	if(filtroMasterG.length > 0){
		for(i=0;i<filtroMasterG.length;i++){
			urlValue = urlValue +'&'+filtroMasterG[i]+'='+dijit.byId('filtro'+i).attr('value');
			if(i!=3){dojo.byId('mostrarFiltros').innerHTML += '<b>' + labelFiltroMasterG[i] + '</b>' + dijit.byId('filtro'+i).item.detalle[0] + ' ';}
			else{dojo.byId('mostrarFiltros').innerHTML += '<b>' + labelFiltroMasterG[i] + '</b>' + dijit.byId('filtro'+i).attr('value');}
		}
	}
	gridMaster.setStore(new dojo.data.ItemFileWriteStore({url:urlValue,urlPreventCache:true}));
	gridMaster.attr('autoWidth',true);
