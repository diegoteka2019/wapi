...
//DMLs
var dmlBuscarMaster = '12504';
var dmlNuevoMaster = '12501';
var dmlEnviarMaster = '12519';
var dmlEditarMaster = '12502';
var dmlEnviarEditarMaster = '12522';
var dmlCopiarMaster = '';
var dmlBuscarUnicoMaster = '12520'; 
var dmleliminarM = '12503';
...
var columnasE = new Array('comprobante_sp_id','area','subregion','cliente','fecha','comprobante','estado','importe','adjunto','observaciones','seleccionado','adjunto_masivo');
...
//DESCRIPCIONES
var descripcionesM = new Array('N&uacute;mero','Area','Sub Regi&oacute;n','Cliente','Fecha','Comprobante','Estado','Importe','Adjunto','Observaciones','Selec.','Adjunto Masivo');
...
var selectFiltroMaster = new Array('','12506','12507','12508','','','12509','12510','12511');
//FORMULARIO
...
var selectFormM = new Array('','12512','12528','12529','12526','12513','xxxxx','','','12514','');
...
var layoutMaster = [
	...
	{width: "33px", field: columnasE[11], name: descripcionesM[11],editable:true,styles:'text-align:center;',type:dojox.grid.cells.Bool},
	{width: "33px", field: columnasE[10], name: descripcionesM[10],editable:true,styles:'text-align:center;',type:dojox.grid.cells.Bool},
...
function DojoAddOnLoad(){
	...
	if(dijit.byId('enviarMasivoWMaster')||dijit.byId('adjuntarArchivoMasivo')){columnasM = columnasE;layoutMaster = layoutEnvio;dojo.place('<div id="masivoAdjunto" class="dijitToolbar" style="text-align:right;height:24px;padding-right:1%"><label for="selec">Adjunto Masivo</label><input id="selec" type="checkbox" title="Adjunto Masivo" onclick="CheckColumnMasivo(this);"/><id="masivoEnvio" class="dijitToolbar" style="text-align:right;height:24px"><label for="selec">Selec. Env&iacute;o</label><input id="selec" type="checkbox" title="Env&iacute;o Masivo" onclick="CheckColumn(this);"/></div>',dojo.byId('cabecera'),'last');}	
	
	if(dijit.byId('cargaMasivaMaster')){dijit.byId('cargaMasivaMaster').attr('disabled',false);}
	if(dijit.byId('adjuntarArchivoMasivo')){dijit.byId('adjuntarArchivoMasivo').attr('disabled',false);}
...
function HabilitarBotonesMaster(tipo,bool){
	...
	if(tipo==2){
		...
		if(dijit.byId('adjuntarArchivoMasivo')){dijit.byId('adjuntarArchivoMasivo').attr('disabled',bool);}}
	if(tipo==3){
...
function CargaMasivaMaster(){
	...
	//Creacion de botones
	var botonCargar = new dijit.form.Button({label:'Cargar',type:'button',iconClass:'dijitEditorIcon dijitEditorIconSave',onClick:function(){PopUp('Advertencia','Se van a cargar Comprobantes sin Presupuesto de forma masiva, desea continuar?','GrabarCargaMasivaMaster()');}});
	...
	dialogM = new dijit.Dialog({title:'Carga Masiva de Comprobantes SP',id:'dialogCargaMasivaMaster'});
...
function BajarPlantillaMaster() {
	var urlValue = 'data/carga_masiva_comprobantesSP.xls';
...
//	if(anioActual>parseInt(dijit.byId('anio').attr('value')) || (mesActual>dijit.byId('mes').attr('value') && anioActual==parseInt(dijit.byId('anio').attr('value')))){PopUp('Error','El mes y a&ntilde;o ingresados deben ser mayores o iguales al mes y a&ntilde;o actual');return;}
	...
	var urlValue = 'dbi/comprobantesSP_carga_masiva.php?sid='+gvarSID; //+'&mes='+dijit.byId('mes').attr('value')+'&anio='+dijit.byId('anio').attr('value');
...
function NuevoMaster(){
	...
	tablaForm.addChild(new dojox.form.CheckedMultiSelect({id:'clientes_desc',style:'width:130px;height:58px;',required:requeridoFormM[4],multiple:true,label:(labelFormM[4]+'<button class="dijitButtonNode" onclick="VerDialogoClientes();" type="button">Ver</button>'),readOnly:true}));
	...
	tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[8],maxLength:'40',maxLength:"499",regExp:regexpFormM[1],required:requeridoFormM[8],style:'width:130px;height:58px;',label:labelFormM[8]}));
	...
	tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[10],style:'width:130px;',maxLength:'40',label:labelFormM[10],promptMessage:mensajesFormM[10],readOnly:true,required:requeridoFormM[10]}));
...
function MostrarGrillasMaster(value){
	...
	dijit.byId('tipo_movimiento').store.fetchItemByIdentity({
		...
		onItem:function(item){
			dijit.byId('gridDestinoP').setStore(null);dijit.byId('gridDestinoP').update();
			dijit.byId('gridOrigenP').setStore(null);dijit.byId('gridOrigenP').update();
			dijit.byId('gridOrigenCuenta').setStore(null);dijit.byId('gridOrigenCuenta').update();
			dijit.byId('gridOrigenP').setStore(null);dijit.byId('gridOrigenP').update();
			dijit.byId('gridProductos').setStore(null);dijit.byId('gridProductos').update();
			if(dijit.byId('tipo_movimiento').store.getValue(item,'tipo_producto')=='F'){
				dijit.byId('divFinanciera').destroyDescendants();
				if(dijit.byId('tipo_cbte_id')){dijit.byId('tipo_cbte_id').destroy();}
				tipo_producto = 'F';
				var tablaFinanciera = new dojox.layout.TableContainer({cols:1,labelWidth:'60',style:'text-align:left;'});
				//tablaFinanciera.addChild(new dojox.form.CheckedMultiSelect({style:'width:250px;height:37px;',label:'Imputa a: ',store:new dojo.data.ItemFileReadStore({data:financieraStore}),onChange:function(value){ChangeFinanciera(value);}}));
				...
				ChangeFinanciera('1');
			}if(dijit.byId('tipo_movimiento').store.getValue(item,'tipo_producto')=='C'){
				dijit.byId('divFinanciera').destroyDescendants();
				if(dijit.byId('tipo_cbte_id')){dijit.byId('tipo_cbte_id').destroy();}
				tipo_producto = 'C';
				var tablaFinanciera = new dojox.layout.TableContainer({cols:1,labelWidth:'60',style:'text-align:left;'});
			  //tablaFinanciera.addChild(new dojox.form.CheckedMultiSelect({style:'width:250px;height:37px;',label:'Imputa a: ',store:new dojo.data.ItemFileReadStore({data:financieraStore}),onChange:function(value){ChangeFinanciera(value);}}));
				...
				if(dijit.byId('tipo_cbte_id')){dijit.byId('tipo_cbte_id').destroy();}tablaFinanciera.addChild(new dijit.form.FilteringSelect({id:'tipo_cbte_id',style:'width:260px;',label:'Tipificaci&oacute;n: ',searchAttr:'detalle',promptMessage:'Seleccione una Tipificaci&oacute;n',required:false,store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=12515&cod_canal_venta='+dijit.byId('cod_canal_venta').attr('value')+'&tipo_movimiento='+dijit.byId('tipo_movimiento').store.getValue(item,'tipo_movimiento_id')})}));
				dijit.byId('divFinanciera').attr('content',tablaFinanciera);
				tablaFinanciera.startup();
				dojo.byId('divFin').style.display = 'block';
				ChangeFinanciera('1');
...
function AdjuntarMaster(){
	...
	if(dijit.byId('archivo')){dijit.byId('archivo').destroy();}tablaForm.addChild(new dijit.form.ValidationTextBox({id:'archivo',style:'width:141px;',label:'Archivo: ',promptMessage:'Seleccione un archivo',required:true}));
...
function CargarClientes(){
	...
	var ActualizarMultiselect = function(items,request){
		...
		clientes =dijit.byId('clientes_desc').value;
		dijit.byId('cliente_id').attr('value',clientes);
...
function BuscarClientes(){
	var urlValue ='dbi/q.php?sid='+gvarSID+'&dmlid=12526&subregion_id='+dijit.byId('subregion_id').attr('value')+'&cod_canal_venta='+dijit.byId('cod_canal_venta').attr('value')+'&cliente_id='+dijit.byId('cli_id').valueNode.value+'&razon_social='+dijit.byId('razon').attr('value');
...
function BuscarProductos(){ 
	if(tipo_producto=='F'){var dml='12517';}else if(tipo_producto=='M'){var dml='12516';}else if(tipo_producto=='C'){var dml='12517';}
...
function EditarMasterEA(){
	...
	var LlenarParametros = function(items,request){
		...
		tablaForm.addChild(new dijit.form.ValidationTextBox({id:camposFormM[19],regExp:regexpFormM[1],maxLength:'40',value:items[0]['observaciones'][0],required:requeridoFormM[19],style:'width:130px;height:58px;',label:labelFormM[19]}));
...
function EnviarMasivoWMaster(){
	...
	if(comprobantesChecked==''){Mensaje('Seleccione al menos un Comprobante','error','master');return;}
	//Grabación de datos en la base de datos
	comprobantesChecked = comprobantesChecked.substr(0,comprobantesChecked.length-1);
	var urlValue ='dbi/e_proc_m.php?sid='+gvarSID+'&dmlid=12523&comprobantes='+comprobantesChecked;
	dojo.xhrPost({url:urlValue,handleAs:'json',sync:'true',load:function(response){
		if(response.errcode==0){
			BuscarMaster();
			Mensaje(response.errmsg,'mensaje','master');
...
function VerDetalleMaster(){
	...
	var LlenarParametros = function(items,request){
		...
		tablaForm.addChild(new dijit.form.ValidationTextBox({value:items[0]['tipo_movimiento'][0]+' - '+items[0]['tipo_cbte'][0],style:'width:130px;',readOnly:true,label:labelFormM[9]}));
		if(items[0]['archivo_id'][0]!='' && items[0]['estado'][0]=='ED' ){tablaForm.addChild(new dijit.form.ValidationTextBox({id:'archivo_id',value:items[0]['archivo_id'][0],readOnly:true,style:'width:130px;',label:labelFormM[10]}));
		dojo.place(botonEliminarAdjunto.domNode,dijit.byId("divBotonesD").domNode,'last');
		}else{tablaForm.addChild(new dijit.form.ValidationTextBox({id:'archivo_id',value:items[0]['archivo_id'][0],readOnly:true,style:'width:130px;',label:labelFormM[10]}));}
		if(items[0]['tipo_producto'][0]=='F'){
			tablaForm.addChild(new dijit.form.ValidationTextBox({value:items[0]['cuenta_contable_id'][0]+' - '+items[0]['cuenta_contable'][0],readOnly:true,style:'width:130px;',label:'Cuenta Contable: '}));
		}else if(items[0]['tipo_producto'][0]=='M'){
			tablaForm.addChild(new dijit.form.FilteringSelect({value:items[0]['tipo_cbte_id'][0],readOnly:true,style:'width:130px;',label:'Tipificaci&oacute;n: ',searchAttr:'detalle',store:new dojo.data.ItemFileReadStore({url:'dbi/q.php?sid='+gvarSID+'&dmlid=12515&cod_canal_venta='+items[0]['cod_canal_venta'][0]+'&tipo_movimiento='+items[0]['tipo_movimiento'][0]})}));
	...
	camposStore.fetch({query:{},onComplete:LlenarParametros});
	//Creacion de botones
	var botonEliminarAdjunto = new dijit.form.Button({id:'botonEliminarAdjunto',label:'Eliminar Adjunto',type:'button',iconClass:'eliminarIcon',onClick:function(){EliminarAdjuntoDetalleAcre();}});
...
function CancelarDetail(){
	...
	if(dijit.byId('archivo_id')){dijit.byId('archivo_id').destroyRecursive();};
	if(dijit.byId('botonEliminarAdjunto')){dijit.byId('botonEliminarAdjunto').destroyRecursive();};
	BuscarMaster();
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
function BajarAdjunto(){
	...
	dijit.byId('gridMaster').store.fetchItemByIdentity({identity:idCbte,onItem:function(item){
		...
		if (adjValue == "on"){
			...
			//document.location = urlValue;
			window.open(urlValue,"Bajar_Adjunto","menubar=no,width=300,height=300,toolbar=no,resizable=yes");
		} else {
...
function CheckColumnMasivo(checkbox){
	...
	//var Checkear = function(item){dijit.byId('gridMaster').store.setValue(item,'seleccionado',checkbox.checked);}
	//dijit.byId('gridMaster').store.fetch({query:{},onItem:Checkear,onComplete:function(){dijit.byId('gridMaster').store.save();}});
	if(dijit.byId('adjuntarArchivoMasivo')){
		dijit.byId('adjuntarArchivoMasivo').attr('disabled',false);
	}
			if(dijit.byId('gridMaster').store){
				var Checkear = function(item){
					if(item['estado'][0]=='ED' && item['archivo_id'][0]==''){
						dijit.byId('gridMaster').store.setValue(item,'adjunto_masivo',checkbox.checked);
					}
				}
				dijit.byId('gridMaster').store.fetch({query:{},onItem:Checkear,onComplete:function(){dijit.byId('gridMaster').store.save();}});
			}
	...
	var archivo_id = '';
		if(dijit.byId('gridMaster').store){
			var ObtenerAdjuntos = function(item){
				adjuntosChecked+=item['comprobante_sp_id'][0]+',';
				estado=item['estado'][0];
				archivo_id=item['archivo_id'][0];
			}
			dijit.byId('gridMaster').store.fetch({query:{adjunto_masivo:'true'},onItem:ObtenerAdjuntos});
		}
		
	if(adjuntosChecked==''){Mensaje('Seleccione al menos un Comprobante','error','master');return;}
	if(estado!='ED'){Mensaje('Debe seleccionar solo Comprobantes con Estado ED','error','master');return;}
	if(archivo_id!=''){Mensaje('Debe seleccionar solo Comprobantes sin Archivo Adjunto','error','master');return;}
...
function AdjuntarMasivoMaster(archivoId){
	...
	var urlValue ='dbi/e_proc_m.php?sid='+gvarSID+'&dmlid=12531&comprobante_sp_id='+adjuntosChecked+'&archivo_id='+archivoId;
	dojo.xhrPost({url:urlValue,handleAs:'json',sync:'true',load:function(response){
		if(response.errcode==0){
			Mensaje(response.errmsg,'mensaje','master');
			BuscarMaster();
...
function EliminarAdjuntoAcre(){
	PopUp('Advertencia','Desea eliminar el Adjunto para el Comprobante: '+registroSeleccionado[0]['comprobante_sp_id'][0]+'?','EliminarAcreMaster()');
}
////////////////////////////////////////////////////////////////////////////////////////////////////
function EliminarAcreMaster(){
	var urlValue ='dbi/e_proc_m.php?sid='+gvarSID+'&dmlid=12530&archivo_id='+registroSeleccionado[0]['archivo_id'][0]+'&comprobante_sp_id='+registroSeleccionado[0]['comprobante_sp_id'][0];
	dojo.xhrPost({url:urlValue,handleAs:'json',sync:'true',load:function(response){
		if(response.errcode==0){
			Mensaje(response.errmsg,'mensaje','master');
			BuscarMaster();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
function EliminarAdjuntoDetalleAcre(){
PopUp('Advertencia','Desea eliminar el Adjunto para el Comprobante: '+registroSeleccionado[0]['comprobante_sp_id'][0]+'?','EliminarAcreDetalle()');
}
////////////////////////////////////////////////////////////////////////////////////////////////////
function EliminarAcreDetalle(){
	var archivo_id = registroSeleccionado[0]['archivo_id'][0];
	var urlValue ='dbi/e_proc_m.php?sid='+gvarSID+'&dmlid=12530&archivo_id='+registroSeleccionado[0]['archivo_id'][0]+'&comprobante_sp_id='+registroSeleccionado[0]['comprobante_sp_id'][0];
	dojo.xhrGet({url:urlValue,handleAs:'json',load:function(response){
		if(response.errcode==0){
		dijit.byId('archivo_id').attr('value','');
		dijit.byId('botonEliminarAdjunto').destroy();
		PopUp('Mensaje', response.errmsg);
		}else{PopUp('Error', response.errmsg);}
	}});
}