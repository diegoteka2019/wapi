//TITULOS
var titulos =  new Array("Marcas Competencia");
//DMLs
var dmlBuscarMaster = "10204";
var dmlnuevoM = "10201";
var dmleditarM = "10202";
var dmlbuscarUnicoM = "10205"; 
var dmleliminarM = "10203";
var dmlAuditoriaMaster = "10206";
var dmlgrabarM = "";
//COLUMNAS
var columnasM = new Array("empresa_competencia_id",'division',"descripcion",'estado');
var tiposM = new Array("text","text","text",'text');
//DESCRIPCIONES
var descripcionesM = new Array('C&oacute;digo','Divisi&oacute;n','Descripci&oacute;n','Habilitada');
//PRIMARY KEY
var clavesM = new Array("empresa_competencia_id");
//FILTROS
var filtroMaster = new Array("area","subregion","division",'marca');
var labelFiltroMaster = new Array("Area: ","Sub-Region: ","Division: ",'Marca: ');
var mensajeFiltroMaster = new Array("Ingrese el area","Ingrese la Sub-Ragion","Ingrese l division","Ingrese la marca");
var tipoFiltroMaster = new Array('FilteringSelect','FilteringSelect','FilteringSelect','FilteringSelect');
//FORMULARIO
var camposFormM = new Array('empresa_competencia_id','descripcion','division_id','estado');
var labelFormM = new Array('C&oacute;digo: ','Descripci&oacute;n: ','Divisi&oacute;n: ','Habilitada: ');
var requeridoFormM = new Array(true,true,true,false);
var regexpFormM = new Array(".*","[a-zA-Z0-9Ò—ø?°! ,.()-_]+",".*",".*");
var mensajesFormM = new Array('Ingrese el c&oacutedigo',"Ingrese la descripci&oacute;n","Ingrese la divisi&oacute;n");
var dojoTypeFormM = new Array('ValidationTextBox','ValidationTextBox','FilteringSelect','CheckBox');
var searchAttrFormM = new Array('','','detalle','');
var selectFormM = new Array('','','10207','');
...
var layoutMaster = [
    ...
    {width: "auto", field: columnasM[3], name: descripcionesM[3],formatter:formatCheckbox,styles:'text-align:center;'}];
//campos de auditoria
...
function NuevoMaster(){
	...
	dijit.byId('empresa_competencia_id').attr('disabled',true);
	dijit.byId('empresa_competencia_id').attr('required',false);
...
function EditarMaster(){
	...
	var gotData = function(items, request){
		for(i=0;i<camposFormM.length;i++){
			...
			if(dojoTypeFormM[i]=='CheckBox'){if(registroCargado.getValue(items[0], camposFormM[i]) == 'false'){dijit.byId(camposFormM[i]).attr('checked',false);}else{dijit.byId(camposFormM[i]).attr('checked',true);}}}
		//Se muestra el dialogo
		dialogM.show();
	}
...
function EliminarMaster(){
	...
	mensaje='Est&aacute; seguro que desea eliminar la marca <b>'+items[0][columnasM[1]]+'</b><br> perteneciente a la divisi&oacute;n '+items[0][columnasM[2]];
...
function GrabarMaster(){
	...
	for(i=0;i<camposFormM.length;i++){
		if(!(dojoTypeFormM[i]=='CheckBox')){
			if(!dijit.byId(camposFormM[i]).isValid()){dijit.byId(camposFormM[i]).focus();return;}
		}
	}
	...
	dijit.byId('descripcion').attr('value',unescape( encodeURIComponent( dijit.byId('descripcion').attr('value') ) ) );
	var urlValue ='dbi/e.php?sid='+gvarSID+'&dmlid='+dmlgrabarM;
	for(i=0;i<camposFormM.length;i++){
		if(dojoTypeFormM[i]=='CheckBox'){urlValue = urlValue +'&'+camposFormM[i]+'='+dijit.byId(camposFormM[i]).attr('checked');}
		else{urlValue = urlValue +'&'+camposFormM[i]+'='+dijit.byId(camposFormM[i]).attr('value');}}
