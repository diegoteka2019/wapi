//TITULOS
var titulos =  new Array('Depuraci&oacute;n de Errores');
//DMLs
var dmlBuscarMaster = '';
var dmlnuevoM = '';
var dmleditarM = '';
...
var dmlgrabarM = '';
var dmldepurarM = '13001'
//COLUMNAS
var columnasM = new Array('logest_id','observaciones','habilitada','nro_doc','id_proceso');
var descripcionesM = new Array('Id Error','Observaci&oacute;n','<input type="checkbox" id="selected" title="Seleccionar Todos" name="selected" onclick="CheckColumn(this);"/> Seleccionar','N&uacute;mero de Comprobante','Tipo de Comprobante');
var clavesM = new Array('');
//FILTROS
var filtroMaster = new Array('id_proceso','nro_doc');
var labelFiltroMaster = new Array('Tipo de Comprobante: ','N&uacute;mero de Documento: ');
var mensajeFiltroMaster = new Array('Seleccione un Tipo de Comprobante','Ingrese un numero de comprobante');
var requeridoFiltroMaster = new Array(true,false);
var tipoFiltroMaster = new Array('FilteringSelect','TextBox');
var searchAttrFiltroMaster = new Array('proceso_desc','');
var selectFiltroMaster = new Array('','');
//FORMULARIO
...
function DepurarMaster(){
	...
	var obtenerValores = function(items, request){
		...
		for(i=0;i<items.length;i++){
			if(dijit.byId('gridMaster').store.getValue(items[i],columnasM[2])){
				if(tablas!=''){tablas+=',';}
				tablas+= dijit.byId('gridMaster').store.getValue(items[i],columnasM[0]);
			}
		}
		if(tablas==''){Mensaje('Seleccione un error','error','master');}
		else{PopUp('Advertencia!', 'Desea depurar los errores seleccionados?', "Depurar('"+tablas+"');");}
	}
...
function Depurar(tablas){
	procesando.show();
	var urlValue ='dbi/e_proc_t.php?sid='+gvarSID+'&dmlid='+dmldepurarM+'&numeros='+tablas+'&id_proceso='+dijit.byId('filtro0').attr('value');
...
function BuscarMaster(){
	var urlValue ='dbi/depuracion_errores_stats.php?sid='+gvarSID;
	if(dijit.byId('filtrMaster')){
...
function CheckColumn(checkbox){
	if(checkbox.checked==true){HabilitarBotonesMaster(2,false);
	}else{HabilitarBotonesMaster(2,true);}
